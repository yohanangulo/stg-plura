  ...
  return (
    <div className="ml-[100px] dark:text-white text-black w-fit dark:bg-muted/60 backdrop-blur-md bg-background/60 !rounded-lg p-2 shadow-2xl">
      <div className="flex items-center flex-1 space-x-2.5">
        <div className={`w-5 h-5 rounded-full  bg-${categoryPayload?.color} rounded`} />
        <div className="w-full">
          <div className="flex items-center justify-between space-x-8">
            <p className="text-right whitespace-nowrap">{categoryPayload.name}</p>
            <p className="font-medium text-right whitespace-nowrap ">{categoryPayload.value}</p>
          </div>
        </div>
      </div>
      {categoryPayload.payload.FunnelPages?.map((page: any) => (
        <div key={page.id} className="dark:text-white/70 text-black flex justify-between items-center">
